...
| End-to-end CLI test | Medium | testing | `init → member add → add → get` in a tempdir; raises `main.rs` quality grade. |
...
| Atomic secret writes | Medium | reliability | `write_secret` uses `fs::write` (non-atomic). Use temp-file + rename. See [../RELIABILITY.md](../RELIABILITY.md). |
